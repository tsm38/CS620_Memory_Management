...
{ 
  ...
  allocatedBytes += bytes;
   /*
HoleNode *temp;             //create a temporary node 
temp = (HoleNode*)malloc(sizeof(HoleNode)); //allocate space for node 
  
 temp->holeSize = bytes;            // store data(first field)
...
head = temp;                  // transfer the address of 'temp' to 'head'
  ...
  */
  ...
  //add 1 space for the process to the process table 
  //processList.numProcesses += 1;
  //holeList
	...
	for (i = 0; i < amount_of_holes; i++)
{
    ...
    if ( holeConductor != 0 )  // Makes sure there is a place to start 
    {
        while(holeConductor != 0)
        { 
		//find lowest start location
		if ((holeConductor->startloc < lowest) &&
		    (holeConductor->printed == 0))
		{
		lowest = holeConductor->startloc;
		}
	...
	//we have the lowest
	//traverse again
	//print out the lowest one that has not already been printed
	holeConductor = holeRoot;
...
//once all printed, reset all printed variables
holeConductor = holeRoot;
    if ( holeConductor != 0 ) 
	{
        while(holeConductor != 0)
